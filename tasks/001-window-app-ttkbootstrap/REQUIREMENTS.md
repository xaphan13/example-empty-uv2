...
- Коммит: не коммитилось — изменения остались в рабочем каталоге (ветка `task-tkinter`)

## Итог

Создан пакет `ex_window_app_ttkbootstrap/` (GUI на ttkbootstrap + pygubu с
запуском 5 курируемых примеров из окна и headless-режимами `--smoke` /
`--smoke-window`), добавлены зависимости и обновлена документация. Все 7
критериев успеха подтверждены прогонами qa; единственный дефект DEF-001
(из adversarial-прогона) исправлен и закрыт.

## Изменения

- `pyproject.toml` — runtime: `ttkbootstrap>=2.2.2`, `pygubu>=0.42.1`; новая dev-группа: `pygubu-designer>=0.46.1`.
- `uv.lock` — перегенерирован под новые зависимости.
- `ex_window_app_ttkbootstrap/__init__.py` — маркер пакета (создан).
- `ex_window_app_ttkbootstrap/example_runner.py` — реестр 5 примеров, `run_example()` с захватом stdout/stderr и логов (`QueueHandler`), headless (создан).
- `ex_window_app_ttkbootstrap/window_app.ui` — XML-разметка Pygubu со stable id виджетов (создан).
- `ex_window_app_ttkbootstrap/application_window.py` — класс `ApplicationWindow`: загрузка `.ui` от `Path(__file__)`, поток + `queue` + `after()`, тёмные темы, фильтр, обработка ошибок (создан).
- `ex_window_app_ttkbootstrap/main_window_app.py` — точка входа: окно / `--smoke [name]` / `--smoke-window [сек]` (создан; после DEF-001 — `type=float` на `--smoke-window`).
- `docs/01_project_structure.md` — пакет в дереве проекта + зависимости (dev-подсекция).
- `docs/02_examples_overview.md` — команды запуска и описание GUI-примера.

## Критерии успеха

| # | Критерий | Результат | Доказательство |
|---|---|---|---|
| 1 | Runtime-зависимости установлены | PASS | e2e/qa_criteria_1_6.txt |
| 2 | Ruff чист по новому пакету | PASS | e2e/qa_criteria_1_6.txt |
| 3 | `--smoke valid_bracket` выводит результат проверки скобок | PASS | e2e/qa_criteria_1_6.txt, e2e/qa_extra_8_11.txt |
| 4 | `.ui`-файл валиден (Builder загружает) | PASS | e2e/qa_criteria_1_6.txt |
| 5 | Импорт `ApplicationWindow` headless, без `Tk()` | PASS | e2e/qa_criteria_1_6.txt |
| 6 | Документация обновлена (grep) | PASS | e2e/qa_criteria_1_6.txt |
| 7 | Окно собирается и закрывается под Xvfb (`--smoke-window 5`) | PASS | e2e/qa_criterion_7.txt |

Дополнительно qa прогнал: `--smoke` всех 5 id, неизвестное имя (exit 2),
ruff по всему проекту, регресс `ex_code_war` / `ex_async_simple` — всё
PASS (e2e/qa_extra_8_11.txt).

## Дефекты

- **DEF-001 (low):** `--smoke-window abc` давал traceback `ValueError`
  вместо ошибки аргумента (источник — ADV-001). Исправлено backend-dev
  (`type=float` в argparse), перепроверено qa — **CLOSED** (DEFECTS.md,
  e2e/qa_adv001_repro.txt, e2e/qa_def001_recheck.txt).

## Adversarial-прогон

Прогнано 13 атак (edge-case аргументы CLI, cwd-зависимость, идемпотентность
раннера, параллельность, Xvfb с экстремальными таймаутами).

- ADV-001: ACCEPTED -> DEF-001 — исправлен, DEF-001 CLOSED.
- Остальные 12 атак — PASS без находок (ADVERSARIAL_REVIEW.md, сводная таблица).
- Наблюдение вне рамок задания (не ADV-запись): `ConfigLogger.LOG_DIR = "./log"`
  создаёт папку логов в произвольном cwd при запуске вне `uv run --project` —
  существующее поведение проекта, не введено этим заданием.

## Участники

- backend-dev: фазы 1–5 (зависимости, раннер + .ui, оконный класс, точка
  входа, документация), фикс DEF-001.
- qa: 11 проверок (7 критериев + 4 дополнительных), заведение и закрытие DEF-001.
- adversary: 13 атак, 1 находка (ADV-001), отчёт ADVERSARIAL_REVIEW.md.
- оркестратор: ревью фаз (возврат фазы 2 на доработку переносов строк),
  триаж ADV-001, ведение DEFECTS.md, архивирование.
